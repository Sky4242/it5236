		...
		/*conn.query(sql, [event.registrationcode], function (err, result) {
		  if (err) {
		    	...
		    	if (result[0].codecount == 0){
					...
					var sql = "INSERT INTO onetimepasswords (otp, userid) " +
		                "VALUES (?, ?)";
					conn.query(sql, [event.otp, event.userid], function (err, result) {
						if (err) {
							...
							//		callback(formatErrorResponse('BAD_REQUEST', ["Email address already registered"]));
				      		...
				      		var sql = "INSERT INTO onetimepasswords (userid, otp) " +
		                    "VALUES (?, ?)";
									conn.query(sql, [event.userid, event.otp, function (err, result) {
								if (err) {
					        		callback(formatErrorResponse('INTERNAL_SERVER_ERROR', [err]));
					      		} else {
						        	console.log("password created");
					      			callback(null,"Password Created");
				      			}
